...
@torch.no_grad()
def se3_to_isometry(se3: SE3):
    """Converts a single SE3 Lie group element to a list of Isometry objects."""
    # This function is not used by the core inference pipeline.
    # We can replace its body to remove the dependency.
    raise NotImplementedError("pycg is not installed, so se3_to_isometry is disabled.")
