...
def main_single(
    ...
    for idx, batch_test in tqdm(enumerate(test_dataloader)):
        ...
        if 'rgb_latents' in batch_test:
            ...
            decoded_video = decode_multi_view_latents(vae, model_input, num_input_multi_views, main_config.vae_backbone)
            # save_video(torch.clamp((decoded_video.detach().cpu().float() + 1) / 2, 0, 1), outdir_raw, name=f'decoded_video_{idx}', fps=out_fps)
            video = None
            decoded_video = None

            print("Done saving decoded video")
        else:
        ...
        # TEST: Here, we override the input videos to be the DiFix videos
        difix_path = "outputs/difix_3"
        # Loads .mp4 view_0.mp4, ..., view_5.mp4 and concatenates them into one video tensor
        difix_videos = []
        for view_idx in range(num_input_multi_views):
            difix_video_path = os.path.join(difix_path, f'view_{view_idx}.mp4')
            reader = imageio.get_reader(difix_video_path, 'ffmpeg')
            frames = []
            for frame in reader:
                frame = torch.from_numpy(frame).permute(2, 0, 1) / 127.5 - 1.0  # Normalize to [-1, 1]
                frames.append(frame)
            frames = torch.stack(frames, dim=0)  # (T, C, H, W)
            difix_videos.append(frames)

        video = torch.stack(difix_videos, dim=1)[None, ...]  # (B, T, V, C, H, W)
        video = einops.rearrange(video, 'b t v c h w -> b (v t) c h w')
        # save_video((video + 1) / 2, outdir_raw, name=f'difix_input_{idx}', fps=out_fps)
        video = video.to(model_input.device).to(model_input.dtype)
        model_input = encode_multi_view_video(vae, video, num_input_multi_views, main_config.vae_backbone)
        batch_test['images_input_embed'] = model_input
        video = None

        print("Done saving difix input video")
